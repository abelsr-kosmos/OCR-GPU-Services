...
class SaveBestModel:
    ...
    def __call__(
        self, model, current_valid_map, epoch, OUT_DIR, config, model_name
    ):
...
def show_tranformed_image(train_loader, device, classes, colors):
    ...
    if len(train_loader) > 0:
        for i in range(2):
            ...
            pred_classes = [
                classes[i] for i in targets[i]["labels"].cpu().numpy()
            ]
            ...
            for box_num, box in enumerate(boxes):
                ...
                cv2.rectangle(sample, p1, p2, color, 2, cv2.LINE_AA)
                w, h = cv2.getTextSize(
                    class_name, 0, fontScale=lw / 3, thickness=tf
                )[
                    0
                ]  # text width, height
                outside = p1[1] - h >= 3
...
def save_validation_results(
    images, detections, counter, out_dir, classes, colors
):
    """
