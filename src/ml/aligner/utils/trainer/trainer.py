    ...
    def train(
        ...
        for img, target, _ in tqdm(self.train_iterator):
            ...
            wandb.log(
                {
                    "batch": logging_batch,
                    "batch_training_loss": loss.cpu().data.numpy(),
                }
            )
            logging_batch += 1
            if lossAvg is None:
...
class Trainer_with_class(GenericTrainer):
        ...
        wandb.log(
            {
                ...
                "train_accuracy": (np.sum(total_corners) / 4)
                / len(total_corners),
                "train_4_corners_accuracy": np.sum(total_corners == 4)
                / len(total_corners),
                "train_3_corners_accuracy": np.sum(total_corners >= 3)
                / len(total_corners),
            }
        )
    ...
    def evaluate_corners(
        ...
        for entry in range(len(target)):
            top_left_y_lower_bound = max(
                0,
                (
                    2 * y_cords[entry, 0]
                    - (y_cords[entry, 3] + y_cords[entry, 0]) / 2
                ),
            )
            top_left_y_upper_bound = (
                y_cords[entry, 3] + y_cords[entry, 0]
            ) / 2
            top_left_x_lower_bound = max(
                0,
                (
                    2 * x_cords[entry, 0]
                    - (x_cords[entry, 1] + x_cords[entry, 0]) / 2
                ),
            )
            top_left_x_upper_bound = (
                x_cords[entry, 1] + x_cords[entry, 0]
            ) / 2

            top_right_y_lower_bound = max(
                0,
                (
                    2 * y_cords[entry, 1]
                    - (y_cords[entry, 1] + y_cords[entry, 2]) / 2
                ),
            )
            top_right_y_upper_bound = (
                y_cords[entry, 1] + y_cords[entry, 2]
            ) / 2
            top_right_x_lower_bound = (
                x_cords[entry, 1] + x_cords[entry, 0]
            ) / 2
            top_right_x_upper_bound = min(
                1,
                (
                    x_cords[entry, 1]
                    + (x_cords[entry, 1] - x_cords[entry, 0]) / 2
            ...
            bottom_right_y_upper_bound = min(
                1,
                (
                    y_cords[entry, 2]
                    + (y_cords[entry, 2] - y_cords[entry, 1]) / 2
                ),
            )
            bottom_right_x_lower_bound = (
                x_cords[entry, 2] + x_cords[entry, 3]
            ) / 2
            bottom_right_x_upper_bound = min(
                1,
                (
                    x_cords[entry, 2]
                    + (x_cords[entry, 2] - x_cords[entry, 3]) / 2
                ),
            )

            bottom_left_y_lower_bound = (
                y_cords[entry, 0] + y_cords[entry, 3]
            ) / 2
            bottom_left_y_upper_bound = min(
                1,
                (
                    y_cords[entry, 3]
                    + (y_cords[entry, 3] - y_cords[entry, 0]) / 2
            ...
            bottom_left_x_upper_bound = (
                x_cords[entry, 3] + x_cords[entry, 2]
            ) / 2
            ...
            tl = self.cordinate_within_intervals(
                top_left,
                (top_left_x_lower_bound, top_left_x_upper_bound),
                (top_left_y_lower_bound, top_left_y_upper_bound),
            )
            tr = self.cordinate_within_intervals(
                top_right,
                (top_right_x_lower_bound, top_right_x_upper_bound),
                (top_right_y_lower_bound, top_right_y_upper_bound),
            )
            br = self.cordinate_within_intervals(
                bottom_right,
                (bottom_right_x_lower_bound, bottom_right_x_upper_bound),
                (bottom_right_y_lower_bound, bottom_right_y_upper_bound),
            )
            bl = self.cordinate_within_intervals(
                bottom_left,
                (bottom_left_x_lower_bound, bottom_left_x_upper_bound),
                (bottom_left_y_lower_bound, bottom_left_y_upper_bound),
            )
            resut_dict = {
                "path": paths[entry],
                "contains_tl": tl,
