...
def get_custom_cost(
    result: Dict, apply_log: bool = True
) -> Union[float, None]:
    """
...
def pair_fields(
    ...
    if texts_x:
        ...
        indices = [(i, j) for i, j in itertools.product(range(last_idx), repeat=2) if j < bboxes_y.shape[0] and i < bboxes_x.shape[0]]
        #for i, j in indices:
        ...
        a_matrices = [np.hstack((bboxes_x_[i], np.ones((4, 1)))) for i,j in indices]
