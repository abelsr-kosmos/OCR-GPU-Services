...
class OCRclassifier(object):
    ...
    def _pair_fields_torch(
        ...
        width_height = np.array(
            [doc_x["width"], doc_x["height"]], dtype=np.float64
        )
        ...
        cost_matrix = np.full(
            (bboxes_x.shape[0], bboxes_y.shape[0]),
            fill_value=max_cost,
            dtype=np.float64,
        )
        ...
        if not texts_x:
            return dict(
                ...
                num_bboxes=dict(
                    total=dict(
                        x=len(doc_x["pages"][0]), y=len(doc_y["pages"][0])
                    ),
        ...
        if len(matching_indices) > 0:
            ...
            A_torch = torch.tensor(
                np.array(a_matrices, dtype=np.float64), device=device
            )
            B_torch = torch.tensor(
                np.array(b_matrices, dtype=np.float64), device=device
            )
        ...
        if remove_ol_method == "z-score":
            ...
            mask_ol = filter_non_outlier_costs(
                cost_assigned, threshold=zscore_threshold
            )
